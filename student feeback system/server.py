import os
import pickle#used to convert csv file to
import pandas as pd
...
#pickele csv to  data frame pandas array sklearn svm class flask 
...
def predict():#predicsts the input using emotions
    ...
    model = pickle.load(open('SVM classifier.pkl', 'rb'))#trained data
    teachingscore = model.predict(pd.array([teaching]))#pd pandas
...
@app.route('/admin')
def root():#individual dashboard display
